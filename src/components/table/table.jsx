...
import { Table, TagPicker, Pagination, Input, InputGroup, SelectPicker } from 'rsuite';
import './table.css'
...
const TableReusable = (props) => {
    ...
    const filterData = (value) => {
        ...
        if (value !== "" && (filterColumn !== "" || filterColumn !== null)) {
            ...
            const filtered = props.data.filter(item => {
                if (item[`${filterColumn}`].toString().includes(value)) {
                    return true;
                }
                return false;
            });
    ...
    return (
        <div className='table'>
            <hr />
            {/* data len {props.data} */}
            <button className='button__actions right' disabled> descargar xlsx</button>
